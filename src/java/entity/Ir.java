...
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "Ir.findAll", query = "SELECT i FROM Ir i"),
    @NamedQuery(name = "Ir.findById", query = "SELECT i FROM Ir i WHERE i.id = :id"),
    @NamedQuery(name = "Ir.findByDescripcion", query = "SELECT i FROM Ir i WHERE i.descripcion = :descripcion")})
public class Ir implements Serializable {
    ...
    public Ir(Integer id) {
        this.id = id;
    }

    public Ir(Integer id, String descripcion) {
        this.id = id;
